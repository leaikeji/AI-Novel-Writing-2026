...
case "$AI_NOVEL_MAINTENANCE_STEP" in
    upgrade-20260912_0055)
        [ "$#" -eq 2 ] \
            && [ "$1" = upgrade ] \
            && [ "$2" = 20260912_0055 ] \
            || fail "upgrade step requires exact target 20260912_0055"
        ;;
    upgrade-20260912_0054)
        [ "$#" -eq 2 ] \
            && [ "$1" = upgrade ] \
            && [ "$2" = 20260912_0054 ] \
            || fail "upgrade step requires exact target 20260912_0054"
        ;;
    upgrade-20260911_0053)
        [ "$#" -eq 2 ] \
            && [ "$1" = upgrade ] \
            && [ "$2" = 20260911_0053 ] \
            || fail "upgrade step requires exact target 20260911_0053"
        ;;
    upgrade-20260910_0052)
        [ "$#" -eq 2 ] \
            && [ "$1" = upgrade ] \
            && [ "$2" = 20260910_0052 ] \
            || fail "upgrade step requires exact target 20260910_0052"
        ;;
    upgrade-20260910_0051)
        [ "$#" -eq 2 ] \
            && [ "$1" = upgrade ] \
            && [ "$2" = 20260910_0051 ] \
            || fail "upgrade step requires exact target 20260910_0051"
        ;;
