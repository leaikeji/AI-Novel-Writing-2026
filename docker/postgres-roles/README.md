...
状态：**计划 53 已接入显式 one-shot maintenance overlay；计划 76 已把 bootstrap、schema-owner migrator 和 validator 的精确门禁扩展到 `20260912_0055`。它不是普通 Compose 启动项。**
...
每次只能使用 `docker compose ... run --rm -T <one-service>` 显式运行一个 service，不得使用 profile-wide `up`。`AI_NOVEL_MAINTENANCE_STEP` 是代码级单步授权：bootstrap 支持精确的 `bootstrap-20260909_0049`／`bootstrap-20260909_0050`／`bootstrap-20260910_0051`／`bootstrap-20260910_0052`／`bootstrap-20260911_0053`／`bootstrap-20260912_0054`／`bootstrap-20260912_0055` 及已审历史 head；validator 使用与显式 head 相等的 `validate-<head>`；migrator 支持精确的 `upgrade-20260909_0050`／`upgrade-20260910_0051`／`upgrade-20260910_0052`／`upgrade-20260911_0053`／`upgrade-20260912_0054`／`upgrade-20260912_0055` 及已审历史步骤。`0050` 至 `0055` 均为前向迁移，不提供 downgrade；需要恢复时使用对应计划的数据库 dump、媒体备份和旧安装快照。不匹配的 service 会在连接数据库前失败。
...
角色校验只接受下列显式已审计 head，不会从仓库、脚本默认值或数据库自身猜测期望版本：
...
| `20260903_0040` | 73 | 历史通用音色池、人物准备与选角表基线 |
...
| `20260912_0054` | 61 | Qwen3-TTS 官方固定音色目录扩展为 9 项；无新增表 |
| `20260912_0055` | 61 | Qwen3-TTS 私人音色普通话约束、设计锚点与 Base 复刻闭包；无新增表 |

`protected-tables.sql` 保存当前 `0055` 的 61 表全集；其中 60 张是 ORM 业务／审计表，`alembic_version` 是唯一系统表。历史 head 使用验证器内冻结的对应集合；`0049` 删除 14 张退役专用表，`0050` 只清理共享函数，`0051` 新增脱敏整书删除审计表，`0052` 新增不可变生命周期事件表，`0053` 只增加备份回执摘要证据列，`0054` 扩展官方音色目录约束与闭包，`0055` 增加私人音色普通话及耐久参考资产闭包。SQL 通过 catalog join 只处理目标库中已存在的表；迁移至候选 head 后必须重新执行 bootstrap并要求对应表集通过。验证器还会拒绝未进入保护清单、也没有非 TTS 理由 allowlist 的 `narration_*`、`voice_*`、`character_*`、媒体和后台任务权威表。
