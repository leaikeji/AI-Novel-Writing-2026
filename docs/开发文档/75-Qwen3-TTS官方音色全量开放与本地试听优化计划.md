...
状态：**V1.0 与 V1.1 均已部署到正式 `18088` 并通过验收。V1.1 已完成旁白页减重、统一命名与试听内存缓存；隔离生命周期中的 Docker Desktop 启动异常按原始失败证据保留，不倒写为通过。**
...
## 13. V1.1 旁白页与试听轻量修复（2026-09-12）

作者要求旁白官方音色不再折叠、整理页面层级、统一名称，并减少重复试听推理。V1.1 只做以下窄改动：

1. 旁白当前声音与官方音色库始终显示，删除“更换／收起旁白音色”状态和按钮；人物音色与高级调音的既有折叠边界不变。
2. 官方音色统一为“官方 speaker｜中文描述”，既有 `qwen.WarmFemale`、`qwen.ClearMale` 产品 ID、provenance、绑定和历史数据均不改写；旧 profile 名仅在页面显示时补齐 speaker。
3. 中文音色卡改为桌面双列、窄桌面单列，减少重复标题和 speaker 文案；保留语言筛选、键盘焦点、当前使用、试听、详情与 Provider 可用性。
4. 固定试听仍先核验作品范围且只调用本地 CustomVoice。首次按 `preset_id + language` 成功合成后，在 PawApp 服务进程内缓存经完整身份／WAV 校验的不可变字节；最多 `9 × 4 = 36` 项，命中时不再调用模型，不写数据库、媒体目录或 Git，也不触发云端。服务重启后缓存自然清空，首次试听重新生成。
5. 本地运行时只在 CustomVoice 未驻留时加载模型；缓存不能被表述为跨重启的预装样音。页面明确区分首次生成与缓存直接播放。

本任务不并行：后端缓存响应头、前端契约、同一音色组件与样式需要同步变更，工作量小且文件边界相互依赖；由主代理串行集成可避免共享契约漂移。验收包括后端缓存 miss→hit 且 Provider 只调用一次、前端契约／局部状态、名称规则、typecheck、build、受影响回归和真实桌面页面。作者已授权并完成正式 `18088` 部署，证据见 [`audit/qwen-tts/plan75/v1.1-formal-release-20260912.md`](../../audit/qwen-tts/plan75/v1.1-formal-release-20260912.md)。

## 14. V1.1 正式发布记录（2026-09-12）

- 候选只由 Git 基线 `902d3e713be0eddbe6f9cf1a769b6e9726787603` 与本节列出的 V1.1 文件组成；未夹带工作区内计划 66／69／70／71／72／74 的施工中改动。
- 正式容器、镜像和数据库 schema 未变；发布不执行迁移，不写正文、Edition、媒体或声音绑定。
- 候选 tree SHA 为 `9e27bac62e8462450b5585f1535dba6befad8210add650771fced9afd18e0bb5`，通过 QwenPaw 公开 `plugin install --force` 安装到原长期容器。
- 启动后应用、数据库、朗读 production backend、worker、catalog v2／9 项、11 个小说 Skill 和 5 个小说工具均通过；Skill 开关与发布前快照一致。
- 正式 HTTP 同一 Ono_Anna／日语试听实测为 `miss 21.440s -> hit 0.027s`，两次 WAV SHA-256 均为 `b55385c357f141e3c70d6281965f27a64cde36311e6158087867d5f63ae1ab63`。
- 真实桌面页面确认旁白音色始终展开、中文双列、统一名称；Ono_Anna 再次试听显示“已直接播放缓存试听”。无新增浏览器 error。
