...
状态：**✅ 2026-08-25 `W5 / UD5-G` 产品修复已通过。中央审阅已按获选方案 1 改为连续 IDE Diff；纯删除具备红色删除与绿色最终结果；引号伪差异和不可验证摘要已在项目边界内消除；真实标题、正文、应用、撤销、自动保存、刷新、历史恢复及 1920×1080、2560×1440、200% 等效缩放均已复核。QwenPaw 2.1.0 公开聊天契约仍不能安全区分并单独隐藏注入上下文，保留已审计降级，不使用私有字段或上游改造。**
...
2026-08-25 按获选设计图与当前运行态进行同状态复查后，确认旧验收存在“自动化通过但产品没有真正对齐”的缺口：

1. 中央审阅仍使用独立圆角卡片、摘要块和逐项底部按钮，未形成方案 1 的单窗口、连续文档式 Diff。
2. MiniMax-M2.7 对 1,067 字正文的一次真实润色返回约 40 处差异，实际主要为半角/全角引号替换；模型摘要却声称进行了节奏和重复动作修改，摘要与可见差异不一致。
3. 纯删除候选只有红色删除行，缺少“应用后的最终结果”提示，容易被误判为绿色结果渲染失败。
4. 页面上下文以 `role=user` 进入 QwenPaw 公开聊天历史是 ADR-0003 已核实边界；是否存在新的公开消息渲染接口可以折叠它，必须先只读核实。不得用私有 DOM、私有 store、上游改造或伪装删除历史来追求表面清爽。

W5 只修复以上审阅视觉、差异降噪、摘要真实性与助手可见边界，不扩展富文本、移动端、第二套聊天或自动应用。

### 12.2 子代理并行施工设计

唯一集成责任人仍为主 Codex；它独占 `frontend/src/styles.ts`、本计划、最终浏览器、QwenPaw 安装态和 Git index。

| 工作包 | 标记 | 唯一目标 | 精确文件/目录所有权 | 前置与退出条件 |
| --- | --- | --- | --- | --- |
| `UD5-FE-DOM` | `PAR-C` | 把 Review Surface DOM 重构为连续 IDE Diff；动作进入变更行右侧；纯删除显示最终结果预览 | `frontend/src/selection-edit-review-surface.ts`、`frontend/src/selection-edit-review-surface.test.ts` | 不改 DTO/runtime/styles；专项 Vitest 通过 |
| `UD5-BE-NOISE` | `PAR-C` | 在保持原文/候选严格可重建的前提下消除纯引号样式伪差异；无实质差异时使用项目可验证摘要 | `backend/selection_edit_diff.py`、`tests/test_selection_edit_diff.py` | 不改 API/schema/迁移/模型策略；专项 pytest 通过 |
| `UD5-CONTEXT-AUDIT` | `PAR` | 只读核实 QwenPaw 2.1.0 公开接口能否隐藏或折叠注入上下文，并给出允许/禁止/降级结论 | 当前源码、公开契约测试、ADR-0003 只读；证据仅写 `docs/开发文档/证据/选区AI中央统一Diff审阅-2026-08-25/UD5-CONTEXT-AUDIT/` | 不操作真实会话、不改上游、不改实现；结论可复核 |
| `UD5-INT` | `SER/INT/MUTEX` | 统一样式、接线、文档状态、浏览器同图对比和真实应用/恢复测试 | `frontend/src/styles.ts`、必要的集成测试、本计划与 `UD5/` 证据；独占浏览器和安装态 | 三个前置包汇合；所有相关自动化通过 |
| `UD5-G` | `GATE/INT` | 重新裁决视觉与生成质量 | 主 Codex | `design-qa.md` 精确写为 `final result: passed` 后才允许交付 |

并行约束：三个前置包不得修改同一文件；不得暂存、提交、推送、修改 QwenPaw 上游、切换 Agent 模型、读写正式用户数据或触碰本计划外的用户改动。接口保持 V2 的 `SelectionEditResultV2`、Review Event、Adapter/CAS、一次应用/撤销语义不变。
...
```bash
pnpm exec vitest run frontend/src/selection-edit-review-surface.test.ts frontend/src/selection-edit-review.test.ts frontend/src/selection-edit-runtime.test.ts
pnpm typecheck
pnpm test
pnpm build
.venv/bin/python -m pytest tests/test_selection_edit_diff.py tests/test_selection_edit_domain_integration.py tests/test_model_runtime.py
.venv/bin/python -m pytest
git diff --check
```

真实验收固定使用“AI 小说作家”当时的有效 MiniMax-M2.7，并在同一本安全样书副本中完成：标题纯删除、标题实质改写、正文 1,000 字以上润色、逐处决定、一次应用、自动保存、刷新、历史/恢复、一步撤销。记录 requested/actual 模型、原文/候选/应用/撤销哈希、真实变更类型与摘要一致性。

视觉验收必须在同一输入中并排放置获选参考图与当前实现截图，覆盖 1920×1080 和 2560×1440。重点复核单窗口、无多余卡片、工具栏单行、动作右对齐、正文连续性、红绿语义、纯删除最终结果、助手折叠/展开、长文滚动、键盘焦点与 200% 等效缩放。最终证据进入 `docs/开发文档/证据/选区AI中央统一Diff审阅-2026-08-25/UD5/`。

### 12.4 当前门禁

- `UD5-OPEN`：✅ 用户已同意开始修复。
- `UD5-FE-DOM`：✅ 连续 Diff DOM、逐处右侧动作与纯删除最终结果已经完成，相关前端测试通过。
- `UD5-BE-NOISE`：✅ 引号字形噪声归一化、真实 Diff 摘要及 12k 输入性能门槛已经完成，相关后端测试通过。
- `UD5-CONTEXT-AUDIT`：✅ 已完成；公开 `ChatRequestData` 没有可信消息来源字段，当前只保留可见上下文、整栏折叠和独立上下文引用，不实施启发式隐藏。
- `UD5-INT`：✅ 支持的插件安装流程、真实 MiniMax-M2.7、正文自动保存/刷新/历史恢复、标题应用/撤销、1920×1080、2560×1440 和 960×540（200% 等效）已经通过。
- `UD5-G`：✅ 同尺寸设计稿/实现对照已复核，根目录 `design-qa.md` 结论为 `final result: passed`。

### 12.5 W5 实际验收摘要

- 支持的安装流程最终执行结果：前端 `38` 个文件、`312` 项通过；后端 `236 passed, 24 skipped`；插件结构校验、安装、健康检查和 Agent/Skill/工具核验通过。跳过项来自当前安装验证环境没有开启对应数据库测试条件，不改写为已执行。
- 真实标题纯删除：`退回的旧木盒 → 退回的木盒`，审阅同时显示红色“删除旧”和绿色“应用后为空”；应用后一步撤销恢复原标题。
- 真实标题改写：`退回的旧木盒 → 雨夜退回的木盒`；Diff 摘要由持久化差异生成，应用后一步撤销恢复原标题。
- 真实正文：对 `1,067` 字正文只执行 `压 → 沉`；一次应用后自动保存为“已保存”，刷新仍存在；随后从生成历史恢复原正文并再次刷新，SHA-256 恢复为 `50e8638661675e79fba021d15a102791874b9d97e48b3890917754a1532a9650`。
- 润色无实质变化时返回“未发现可验证的实质差异”，不再把直/弯引号替换渲染成大量修改。MiniMax 偶发返回多个候选 JSON 时，API 仅在独立会话内自动纠错重试一次；第二次仍不合规则继续 fail closed。
- 视觉与运行证据见 [`UD5/README.md`](./证据/选区AI中央统一Diff审阅-2026-08-25/UD5/README.md)；公开契约结论见 [`UD5-CONTEXT-AUDIT/README.md`](./证据/选区AI中央统一Diff审阅-2026-08-25/UD5-CONTEXT-AUDIT/README.md)。
