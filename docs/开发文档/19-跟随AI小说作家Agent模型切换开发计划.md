...
状态：**计划已完成 2026-08-25 三次审核，是本项目模型切换的唯一批准方案；尚未实施。只有本文阶段 0 契约验证通过后，才能开始运行时、数据库和前端改造。**
...
- [ ] PawApp 后端通过公开 active-model API 读取 effective 模型的同进程调用方式。
...
- [ ] 数据库单向迁移在当前真实历史记录上的 upgrade、中断恢复和前滚修复。
...
**计划在“零模型档案、跟随专用 Agent 有效模型、只有一条生产运行路径”的前提下可以进入阶段 0。**

当前不批准绕过阶段 0 直接上线。必须先证明公开 effective 模型读取、专用 Agent 路由、QwenPaw 可见实际模型审计和单向迁移安全；四项证据齐全后实施唯一方案。任何阶段发现问题都只允许修复该方案，不退回固定 MiniMax-M3、模型档案、双轨开关或静默备用模型。
