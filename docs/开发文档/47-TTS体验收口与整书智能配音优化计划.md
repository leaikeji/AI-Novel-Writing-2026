...
只有两项均通过，才允许表述为“本轮 TTS 优化完成”。原计划文本不自动授权长期部署、Git 提交或推送；源码后来已按用户明确授权提交并推送，长期部署仍需独立授权与发布门禁。
...
计划 47 施工前的线性 head 为 `20260830_0035`；本计划新增后，当前源码 head 已为 `20260901_0036`：
...
- 0036 已有正式记录后禁止降 schema；当前没有已验收的单能力关闭开关，回退必须先让全部 narration fail closed 并进入 `HOLD_0036_COMPAT_PATCH`，保留记录等待兼容补丁。活动命令由租约恢复，无法恢复则 `superseded`，不得修改绑定。
...
- 计划 49 已完成发布前迁移/角色兼容加固并裁决 `RELEASE-READY=PASS`；该结论不改变本计划的作者听感、真实 Provider 和长期部署门禁。
