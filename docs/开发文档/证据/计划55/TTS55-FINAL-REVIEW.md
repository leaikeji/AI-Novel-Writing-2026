...
状态：**最终 PASS。默认旁白、人物卡自动专属音色和中文 24 槽通用音色包均已通过机器、真实模型、桌面浏览器、真实章节与作者听检门禁。**
...
- 长期 PawApp `127.0.0.1:18088`、长期 PostgreSQL 和真实用户小说／媒体均未迁移、未替换、未写入。
- 真实写入只发生在隔离 PostgreSQL、隔离媒体目录、隔离 Sidecar 和 `127.0.0.1:18192` 的临时 PawApp。
- 当前候选未提交、未推送；长期部署、Git 提交和推送仍需各自的明确授权。
