...
状态：**✅ A0A–A7 已于 2026-08-25 全部完成，A7-G 技术与产品门禁通过；V2 实现随后随提交 `6790eb7` 推送到 `AI-Novel-Writing-2026/main`。本文后续保留的“尚未提交/待 Git 授权”措辞是 A7-G 当时的点时证据，不再代表当前发布状态。**

后续继任说明：本文保留 2026-08-25 V2 的历史实现与验收事实，不重新开启 A0A–A7。用户随后确认以中央统一 Diff 取代 Clipboard 默认交接和聊天内完整候选卡片；新增范围见 [选区 AI 中央统一 Diff 审阅施工计划](./21-选区AI中央统一Diff审阅施工计划.md)与 [ADR-0004](./ADR/ADR-0004-选区AI编辑任务与中央统一Diff审阅.md)。该继任计划已进入施工，当前仍以其 W2/W3 门禁为准，不能把施工中候选写成最终验收完成。
...
- Git HEAD 与 `AI-Novel-Writing-2026/main` 均为 a4007ab；模型跟随施工提交为 0245bd1，专项文档内并行治理提交为 a4007ab。A1–A7 当前实现仍是未提交工作区候选，不冒充已发布提交。
...
| --- | --- | --- | --- | --- | --- | --- |
| A0A-1 | `DONE` | 基线审计 | 确认 UI/Git 基线 | 无 | ✅ | HEAD 与 `AI-Novel-Writing-2026/main` 均为 a4007ab |
