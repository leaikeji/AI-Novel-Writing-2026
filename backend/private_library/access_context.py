"""Plan74 P0 access candidate, deliberately NOT registered by the plugin.

Reuses the existing context-ref registry and public on_reply boundary. A page
lease proves scope, not that the author authorized any mutation. No database,
tools, Skill registration, HTTP route or model call is implemented here.
"""
...
class LibraryAccessProbeMiddleware(MiddlewareBase):
    ...
    async def on_reply(self, agent: Any, input_kwargs: dict[str, Any],
        ...
        try:
            while True:
                token = _CURRENT_ACCESS.set(evidence)
                try:
                    try:
...
def create_library_access_probe(
    ...
    """Only the integration owner may enable the unpublished P0 candidate."""
    del agent_config
    ...
    if (
        ...
        or getattr(request, "session_id", None) not in (None, session_id)
        or getattr(request, "agent_id", None) not in (None, TARGET_AGENT_ID)
