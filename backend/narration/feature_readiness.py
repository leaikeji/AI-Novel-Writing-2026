"""Atomic, fail-closed readiness for the three TTS35 runtime features.
...
class NarrationFeatureReadinessSnapshot(BaseModel):
    ...
    def validate_snapshot_shape(self) -> "NarrationFeatureReadinessSnapshot":
        ...
        if keys != MANAGED_CAPABILITY_KEYS:
            raise ValueError(
                "readiness capabilities must contain the three managed keys in order"
            )
        enabled = tuple(
...
class NarrationFeatureReadinessProvider:
    """The single atomic control plane for TTS35 feature readiness.
    ...
    def publish_dependencies(
        ...
        *,
        ...
        """Atomically replace all three decisions from one probe result."""
