"""Batch-one MPS adapter for the official MOSS-TTS delay generation loop.

The official 97521ec2 implementation uses a chained boolean-index assignment
for text-logit masking.  PyTorch 2.9.1 can turn that temporary view into an MPS
placeholder without storage.  This adapter preserves the official forward and
sampling algorithm but replaces that expression with an in-place mask on the
single supported batch row.  It never enables CPU fallback.
"""
...
SCHEMA = "vg40-mps-generation-adapter/2"


class MPSGenerationAdapterError(RuntimeError):
    pass
...
def generate_batch_one(
    ...
    *,
    ...
    if getattr(input_ids, "device", None).type != "mps":
        raise MPSGenerationAdapterError("input IDs must be on MPS")
    if getattr(attention_mask, "device", None).type != "mps":
        raise MPSGenerationAdapterError("attention mask must be on MPS")
    batch_size, seq_len, channels = input_ids.shape
    if batch_size != 1 or channels != 17:
        raise MPSGenerationAdapterError("adapter supports exactly batch=1 and n_vq=16")
    if not 1 <= max_new_tokens <= 256:
        raise MPSGenerationAdapterError("generation length exceeds the spike bound")
    ...
    if not callable(sample_token) or not callable(find_last_equal):
        raise MPSGenerationAdapterError("official generation helpers are unavailable")
    ...
    for time_step in range(max_new_tokens):
        ...
        next_token_logits = [
            logit[:, -1, :]
            / (text_temperature if index == 0 else audio_temperature)
        ...
        # MPS compatibility delta: avoid x[mask] = x[mask].index_fill(...),
        # whose temporary can have no MPS storage in PyTorch 2.9.1.
        ...
        if time_step == 0:
            text_logits[..., model.config.audio_assistant_delay_slot_token_id] = float(
                "-inf"
            )
        if time_step <= n_vq:
        ...
        if bool(sampling_audio_mask.any().item()):
            if bool(sampling_audio_mask[0, 0].item()):
                audio_ch0_logits = next_token_logits[1]
                audio_ch0_logits[..., model.config.audio_pad_code] = float("-inf")
            ...
            for channel in range(1, n_vq):
                if not bool(sampling_audio_mask[0, channel].item()):
                    continue
                channel_logits = next_token_logits[channel + 1]
                channel_logits[..., model.config.audio_pad_code] = float("-inf")
        ...
        # The voice asset is complete once all codec channels have flushed and
        # the official audio_end token is emitted.  Waiting for the later chat
        # im_end marker can produce an unbounded text tail for some seeds and
        # does not add audio samples.
        if bool(audio_completed[0].item()) or bool(is_stopping[0].item()):
            break

    start_indices = find_last_equal(
        input_ids[..., 0], model.config.im_start_token_id
    ) + 3
    start_lengths = seq_len - start_indices
    start_index = int(start_indices[0].item())
    start_length = int(start_lengths[0].item())
