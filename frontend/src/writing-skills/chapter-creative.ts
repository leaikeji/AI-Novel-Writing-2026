/** Managed chapter-creation helpers with exact draft-version recovery. */
...
export const CHAPTER_CREATIVE_METHOD_KINDS = [
  "chapter_storyline_recommendation",
  "chapter_outline",
] as const;
export type ChapterCreativeMethodKind = typeof CHAPTER_CREATIVE_METHOD_KINDS[number];
...
export interface ChapterCreativeMethodCatalog {
  readonly available: boolean;
  readonly catalogAvailable: boolean;
  readonly displayNames: Readonly<Record<string, string>>;
  readonly binding: WritingActionBinding;
  readonly chapterDraftId: string;
  readonly chapterDraftVersion: number;
  readonly kind: ChapterCreativeMethodKind;
}

export async function chapterCreativeMethodCatalog(
  novelId: string,
  chapterDraftId: string,
  chapterDraftVersion: number,
  kind: ChapterCreativeMethodKind,
  tabId: string,
  request: Request = apiRequest,
): Promise<ChapterCreativeMethodCatalog> {
  if (!Number.isSafeInteger(chapterDraftVersion) || chapterDraftVersion < 1) {
    throw new Error("章节草稿版本无效");
  }
  ...
  if (data?.schema_version !== "writing-skill-catalog/1"
    ...
    throw new Error("章节写作方法目录或小说范围无法确认");
  }
...
export interface ChapterCreativeMethodInput {
  readonly kind: ChapterCreativeMethodKind;
...
export function managedChapterCreativeInputSnapshot(
  kind: ChapterCreativeMethodKind,
  input: Readonly<Record<string, unknown>>,
): Readonly<Record<string, unknown>> {
  if (kind === "chapter_storyline_recommendation") return Object.freeze({});
  const attempt = input.rewrite_attempt;
  if (!Number.isSafeInteger(attempt) || Number(attempt) < 1 || Number(attempt) > 3) {
    throw new Error("章纲重写次数无效");
  }
  return Object.freeze({ rewrite_attempt: attempt });
}

export class ChapterCreativeMethodClient {
  private readonly core: ManagedCreativeMethodClient;

  constructor(
    readonly catalog: ChapterCreativeMethodCatalog,
    request: Request = apiRequest,
    ...
    this.core = new ManagedCreativeMethodClient(
      catalog,
      {
        ticketNamespace: `anw-chapter-creative-action/1:${catalog.kind}:${catalog.chapterDraftId}:v${catalog.chapterDraftVersion}`,
        allowedKinds: new Set(CHAPTER_CREATIVE_METHOD_KINDS),
        validateScope: input => (
          input.scope_type === "chapter_creation"
          && input.scope_id === catalog.chapterDraftId
          && input.novel_id === catalog.binding.scopeId
          && input.document_id === undefined
          && input.kind === catalog.kind
          && input.expected_scope_version === catalog.chapterDraftVersion
        ),
        validateResultScope: result => result.scope_id === catalog.chapterDraftId,
  ...
  start(input: ChapterCreativeMethodInput): Promise<ManagedCreativeResult> {
    return this.core.start({
      scope_type: "chapter_creation",
      scope_id: this.catalog.chapterDraftId,
      novel_id: this.catalog.binding.scopeId,
      kind: input.kind,
      input_snapshot: managedChapterCreativeInputSnapshot(
        input.kind,
