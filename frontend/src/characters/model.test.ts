...
describe("character workspace model", () => {
  ...
  it("builds one atomic V2 CAS command for changed aggregates", () => {
    ...
    const command = buildSaveCommand(workspace, root, profile, "character-workspace:test");

    expect(command.schema_version).toBe("character-workspace-save/2");
    expect(command.operation_key).toBe("character-workspace:test");
    ...
    expect(command.root_patch?.name).toBe("林舟（改名后）");
