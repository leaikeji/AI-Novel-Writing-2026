...
## 正式写作发现F01后的补充（2026-09-13）

`SER L74R-CAPTURE-FIX`由主代理独占`backend/creative_data_api.py`的capture入口、`backend/private_library/service.py`新增绑定辅助函数及`tests/private_library/test_capture_saved_binding.py`；不并行，原因是资产保存与绑定共享一个事务。本包是既有快速入库承诺的缺陷修复，不改DTO、schema、Skill或普通启停语义。前置为正式F01证据，汇合仍按G2-A→精确提交→G2-B→新备份与公开更新→正式复验；Git／正式环境锁归主代理，其他任务与所有未分配文件只读。

- “保存并用于本书”必须原子保存并绑定这次保存的精确版本；不能只检查是否已有绑定。保留目标原有required／preferred及位置，其他资料和其他作品绑定保持。
- 重复启用仍不推进固定版本；资产写入重放也不得覆盖作者后续停用。返回`used_by_current_novel`按当前精确绑定计算。
- 当前启用版本落后于本次保存基线时返回可恢复409并整体回滚，不能借一条选区收藏隐式启用整包尚未采用的变更。作者先审阅并明确更新绑定，或仅收藏；原文、历史版本及未采用修改均保留。
- 锁顺序小说→资产→绑定，资产与绑定写入同事务。临时PostgreSQL覆盖首次／重复收藏、四种用法、版本落后、他书／归档绑定保留、重放及故障回滚；正式复验必须核对生效版本，不只看UI“已启用”。
