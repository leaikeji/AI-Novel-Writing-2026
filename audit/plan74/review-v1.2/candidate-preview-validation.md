...
日期：2026-09-13 22:40（Asia/Shanghai）。正式源仍为15f4f5f；本轮没有代码部署、重启、迁移或模型配置变更。状态：候选交互与恢复分项PASS；当时内嵌浏览器完整2K截图受限，23:03已由独立Edge补证关闭，真实长列表R5继续开放。
...
23:03补证：正式源6837e1b中使用独立Edge取得[原生2560×1440候选逐项检查截图](desktop/52-candidate-preview-edge-2560x1440.jpg)，CSS／DOM／JPEG均2560×1440、DPR 1；同一临时页的[完整历史列表](desktop/51-edge-history-2560x1440.jpg)显示扫描2处／当前待处理1。正文、job和绑定与F21后证据完全相同，未保存新决定或采用。故下表内嵌浏览器2450宽限制保留为22:40历史，但不再构成当前完整2K候选截图缺口。
