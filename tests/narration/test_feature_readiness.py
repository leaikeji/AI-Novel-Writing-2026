...
@pytest.mark.parametrize(
    ...
    (
        ("schema_ready", ALL_FEATURES, TTS_DATABASE_SCHEMA_OUTDATED),
        (
        ...
        (
            "storage_ready",
            frozenset({ADVANCED_TUNING, PRIVATE_DELETION}),
            TTS_STORAGE_UNAVAILABLE,
        ),
        (
...
def test_all_dependencies_ready_enables_exactly_the_three_frozen_features() -> None:
    provider = NarrationFeatureReadinessProvider()
    starting = provider.begin_startup()
    ...
    with pytest.raises(KeyError):
        ready.item(wire.CapabilityKey.VOICE_GENERATOR)
...
def test_illegal_transitions_and_generation_types_fail_without_publication() -> None:
    ...
    with pytest.raises(ValueError, match="managed keys"):
        provider.mark_crashed([wire.CapabilityKey.VOICE_GENERATOR])
    with pytest.raises(ValueError, match="managed keys"):
