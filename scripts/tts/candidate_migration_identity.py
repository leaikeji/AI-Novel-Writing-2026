...
def _canonical_candidate_root(candidate_root: Path) -> Path:
    ...
    if resolved != candidate_root or candidate_root.is_symlink() or not candidate_root.is_dir():
...
def _validate_alembic_config(candidate_root: Path) -> None:
    ...
    if parser.get("alembic", "script_location", fallback="").strip() != "backend/migrations":
        raise CandidateMigrationError("ALEMBIC_SCRIPT_LOCATION_INVALID")
    version_locations = parser.get("alembic", "version_locations", fallback="").strip()
...
def _parse_migration(path: Path, raw: bytes) -> _MigrationNode:
    try:
        tree = ast.parse(raw.decode("utf-8"), filename=path.name, mode="exec")
    ...
    for statement in tree.body:
        ...
        try:
            ...
            raise CandidateMigrationError("MIGRATION_IDENTITY_NOT_LITERAL", path.name) from error
    ...
    if down_revision is not None and (
        type(down_revision) is not str or REVISION_RE.fullmatch(down_revision) is None
    ):
