...
- 可信任务封套若带本书结构化用词规则，推荐词不是必须插入项；慎用词只有达到所列密度阈值才构成提醒；禁用词按精确短语或 ASCII 完整词处理。不要把单个“像 AI 的词”当成人类／AI 来源判定，也不要用机械替换抹平人物口吻。
- PawApp 的本地检查报告才是命中位置和完整性证据。报告缺失或标记未完成时不得声称全文通过；作者逐条保留只对当次候选生效，不改写词库。
