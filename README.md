...
2026-09-13当前私有库状态：[计划74](./docs/开发文档/74-私有库分类用词与写作接入规划.md#14-v12复查优化与可靠交付计划)V1.2完整源`1712883`已公开热更新，PawApp `0.4.0`／schema `20260913_0056`／逐Agent选择保持；来源与正式测试授权阻断已解除。收藏与生效版本同步、归档停用、受控稿恢复保存、桌面布局及助手撤销已取得分项证据；原句定位滚动及明确维护指令识别修复已提交`77870d4`，尚待正式复验，G3／最终交付未关闭。当前桌面端范围和[施工／历史证据更正](./audit/plan74/README.md)优先于历史记录；AI文学效果没有最优结论。
